...
export default function MoodHeatmap({ logs }: MoodHeatmapProps) {
    ...
    const [selectedLog, setSelectedLog] = React.useState<MoodLog | null>(null);
    ...
    const days: { date: Date; rating?: number; count: number; log?: MoodLog }[] = [];
    ...
    while (loopDate <= today) {
        ...
        days.push({
            ...
            count: log ? 1 : 0,
            log: log // Store the log for the modal
        });
    ...
    // Helper to get color (0 = No entry, 1-5 Scale)
    const getColor = (rating: number) => {
        if (!rating) return 'bg-[#1a2e1a]'; // Empty/Placeholder
        switch (rating) {
            ...
            case 4: return 'bg-[#4ade80] shadow-[0_0_8px_rgba(74,222,128,0.6)]';    // Good
            case 5: return 'bg-[#22c55e] shadow-[0_0_12px_rgba(34,197,94,0.8)] border border-green-300';     // Great
        ...
        <div className="w-full overflow-x-auto pb-2">
                        ...
                        {weeks.map((week, wIndex) => (
                            <div key={wIndex} className="flex flex-col gap-1">
                                {week.map((day, dIndex) => (
                                    <div
                                        key={dIndex}
                                        onClick={() => day.log && setSelectedLog(day.log)}
            ...
            {selectedLog && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setSelectedLog(null)}>
                    <div
                        className="bg-[#031207] border border-gray-800 rounded-2xl p-6 w-full max-w-md shadow-2xl relative"
                    ...
                    >
                        ...
                        <div className="flex flex-col items-center gap-2 mb-8">
                            <div className="text-6xl animate-bounce-slow">
                                {['', '😫', '☹️', '😐', '🙂', '🤩'][selectedLog.rating]}
                            </div>
                            <h3 className={`text-2xl font-bold ${selectedLog.rating === 1 ? 'text-red-500' :
                                    selectedLog.rating === 2 ? 'text-orange-500' :
                                        selectedLog.rating === 3 ? 'text-yellow-500' :
                                            selectedLog.rating === 4 ? 'text-green-400' :
                                                'text-green-500'
                                }`}>
                                {['', 'Awful', 'Bad', 'Okay', 'Good', 'Great'][selectedLog.rating]}
                            </h3>
                        </div>
                        ...
                        <div className="space-y-4">
                            {selectedLog.summary && (
                                <div className="bg-[#0F1E0F] p-4 rounded-xl border border-gray-800">
                                    <p className="text-gray-200 italic text-center">"{selectedLog.summary}"</p>
                                </div>
                            )}

                            {selectedLog.note && (
                                <div>
                                    <h4 className="text-sm font-semibold text-gray-400 mb-2 uppercase">Notes</h4>
                                    <p className="text-gray-300 leading-relaxed whitespace-pre-wrap">{selectedLog.note}</p>
                                </div>
                            )}

                            {!selectedLog.summary && !selectedLog.note && (
                                <p className="text-gray-500 text-center italic">No additional details recorded.</p>
                            )}
                        </div>
                    </div>
                </div>
